...
public class OfficeServiceImpl extends ServiceImpl<OfficeDao, Office> implements OfficeService {
  ...
  @Override
  public List<Office> selectList(Boolean isEnable, Boolean isParent, Integer currentId,
      String prefix) {
    List<Office> list = dao.getChildNodes(currentId);

    if (!StringUtils.isEmpty(prefix)) {
      List<Office> dataList = Lists.newArrayList();
      for (Office office : list) {
        office.setName(prefix + office.getName());
        dataList.add(office);
  ...
  private void buildTree(Integer officeId, List<Office> list, List<Office> dataList,
      String currentPrefix, String prefix) {
    List<Office> subList = dao.getChildNodes(officeId);
    if (!subList.isEmpty()) {
      for (Office office : subList) {
        office.setName(prefix + currentPrefix + office.getName());
        dataList.add(office);
