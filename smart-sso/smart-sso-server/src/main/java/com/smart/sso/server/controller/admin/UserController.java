...
public class UserController extends BaseController {
  ...
  public Result list(@ValidateParam(name = "登录名") String account,
      @ValidateParam(name = "姓名") String name, @ValidateParam(name = "机构ID") Integer officeId,
    ...
    Office myOffice = officeService.selectById(me.getOfficeId());
    if (officeId == null) {
      officeId = myOffice.getId();
    }
    
    return Result.createSuccess(
        userService.selectPage(account, name, officeId, Page.create(pageNo, pageSize)));
  }

  @ApiOperation("验证登录名")
  @ResponseBody
  @RequestMapping(value = "/validateAccount", method = RequestMethod.POST)
  public Result validateAccount(@ValidateParam(name = "id") Integer id,
  ...
  public List<Office> officeTree() {
    ...
    office.setId(myOffice.getId());
    office.setParentId(myOffice.getId());
    office.setName("机构");
    list.add(0, office);
    return list;
  }
